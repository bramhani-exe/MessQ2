...
export default function App() {
  ...
  const liveCfg = iotService.getLiveConfig();

  const handleConnectLive = (mode: 'server' | 'direct', serverUrl: string, esp32Url: string) => {
    iotService.configureLiveSource({ mode, serverUrl, esp32Url, enable: true });
  };

  const handleDisconnectLive = () => {
    iotService.disconnectLive();
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col selection:bg-cyan-500/30 selection:text-cyan-200">
      ...
      <DemoControlsModal
        ...
        liveMode={liveCfg.mode}
        serverUrl={liveCfg.serverUrl}
        esp32Url={liveCfg.esp32Url}
        dataSource={snapshot.dataSource}
